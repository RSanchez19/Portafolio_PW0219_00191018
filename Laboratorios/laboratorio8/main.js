...
let new_cell_input = document.createElement("td")
let new_input = document.createElement("input")

new_input.className = "input-carnet"


new_btn.addEventListener("click", event =>{
let id_actual = event.target.value
let student_carnet = element.carnet

student_list.forEach((element, pos)=>{
if(id_actual == element.id && student_carnet == new_input.value){
student_list.splice(pos,1)
printArray()
}else{
    alert("¡DIGITE EL NUMERO DE CARNET A ELIMINAR!")
}
})
})
...
new_cell_input.appendChild(new_input)
new_row.appendChild(new_cell_input)
table_body.appendChild(new_row)
})
}
